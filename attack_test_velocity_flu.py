...
fake_yolo_position = Vector3()

fake_yolo_position.x = 0
fake_yolo_position.y = 0
fake_yolo_position.z = 1.0

# 飞向target
def go_to_target(adj_kp=1.5, adj_kd=3.76):
    global fake_yolo_position
    global err_x0, err_y0, err_z0
    global velocity

    rospy.loginfo("Go to fake yolo target x: {} y: {} z: {}".format(
        fake_yolo_position.z,
        fake_yolo_position.x,
        fake_yolo_position.y
    ))

    target_x = fake_yolo_position.z  # Forward or Backward
    target_y = fake_yolo_position.x  # Left or Right
    target_z = fake_yolo_position.y  # Up or Down
    ...
    while(not rospy.is_shutdown()):
        ...
        if(current_state.mode != 'OFFBOARD' and (rospy.Time.now() - last_req) > rospy.Duration(5.0)):
            ...
            if mission_step == 0:
                ...
                if (rospy.Time.now() - last_req) > rospy.Duration(2.5):
                    rospy.logwarn_once("Attack finished.")
                    mission_step = 2
                    set_mav_frame_client(1)
                    last_req = rospy.Time.now()
                
                fake_yolo_position.x = random.uniform(0.9, 1.0)
                fake_yolo_position.y = random.uniform(0.0, 0.1)
                fake_yolo_position.z = 1.5
                go_to_target()
